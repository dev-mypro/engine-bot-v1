...
class TradingBot:
    def __init__(self, config: Dict, analyzer, trader, gemini_client=None):
        ...
        if account:
            ...
            self.starting_balance = config.get("current", {}).get(
        ...
        self.enable_multi_symbol = config.get("current", {}).get(
            "enable_multi_symbol", False
        )
        self.enable_multi_timeframe = config.get("current", {}).get(
            "enable_multi_timeframe", False
        )
        self.symbols_to_trade = config.get("current", {}).get(
            "symbols_to_trade", [config["current"]["symbol"]]
        )
        self.timeframes_to_check = config.get("current", {}).get(
            "timeframes_to_check", ["M5"]
        )
        ...
        # Dynamic lot sizing
        self.dynamic_lot_sizing = config.get("current", {}).get(
        ...
        # Check interval
        self.check_interval = (
            config.get("current", {}).get("auto_analyze_interval", 1) * 60
        )
        ...
        print(
            f"   Auto-close profit: ${config.get('current', {}).get('auto_close_target', 0.4):.2f}"
        )
    ...
    def start(self) -> None:
        ...
        print(
            f"🛑 MAX {self.tracker.max_total_positions} POSITIONS - Will STOP when full!"
        )
    ...
    def stop(self) -> None:
        ...
        print(
            f"   Trades executed: {self.tracker.trades_today}/{self.tracker.max_daily_trades}"
        )
    ...
    def _get_market_data(self) -> pd.DataFrame:
        return self._get_market_data_for(
            self.config["current"]["symbol"], self.config["current"]["timeframe"]
        )
