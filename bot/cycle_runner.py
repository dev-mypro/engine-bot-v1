...
class CycleRunner:
    ...
    def run_rapid_fire_cycle(self) -> None:
        ...
        try:
            ...
            if total_open >= self.bot.tracker.max_total_positions:
                ...
                print(f"[{timestamp}] ⚠️ MAX POSITIONS REACHED ({total_open}/{self.bot.tracker.max_total_positions}) - Waiting for close...")
                
                if total_open > 0:
                    total_profit = sum(p.profit for p in current_positions)
                    print(f"           Open P/L: ${total_profit:+.2f} | Target: ${self.bot.config['current']['auto_close_target']:.2f}")
            ...
            if self.bot.tracker.trades_today >= self.bot.tracker.max_daily_trades:
                print(f"⚠️ Daily limit reached ({self.bot.tracker.trades_today}/{self.bot.tracker.max_daily_trades})")
                return
            
            symbols = self.bot.symbols_to_trade if self.bot.enable_multi_symbol else [self.bot.config['current']['symbol']]
            ...
            timestamp = datetime.now().strftime('%H:%M:%S')
            print(f"[{timestamp}] Analyzing {len(symbols)} symbols | Positions: {total_open}/{self.bot.tracker.max_total_positions} | Slots: {remaining_slots}")
                    ...
                    if self.bot.enable_multi_timeframe:
                        for tf in self.bot.timeframes_to_check:
                            future = executor.submit(
                                ...
                                symbol, tf, self.bot.analyzer, self.bot.config, self.bot._get_market_data_for
                            )
            ...
            if signals and remaining_slots > 0:
                ...
                for signal in signals_to_execute:
                    if execute_signal(
                        signal, self.bot.trader, self.bot.tracker, self.bot.config, 
                        self.bot.starting_balance, self.bot.dynamic_lot_sizing, self.bot.risk_percent
                    ):
                        executed += 1
    ...
    def run_cycle(self) -> None:
        ...
        try:
            ...
            if total_open >= self.bot.tracker.max_total_positions:
                print(f"⚠️ Max total positions reached ({total_open}/{self.bot.tracker.max_total_positions})")
                return
            
            symbol_positions = len([p for p in current_positions if p.symbol == self.bot.config['current']['symbol']])
            
            if symbol_positions >= self.bot.tracker.max_positions_per_symbol:
                print(f"⚠️ Max positions for {self.bot.config['current']['symbol']} ({symbol_positions}/{self.bot.tracker.max_positions_per_symbol})")
                return
            ...
            analysis = self.bot.analyzer.analyze_market(
                df,
                self.bot.config['current']['symbol'],
                self.bot.config
            )
            ...
            if self.bot._should_trade() and signal != 'WAIT':
                execute_signal(
                    ...
                    self.bot.trader, self.bot.tracker, self.bot.config,
                    self.bot.starting_balance, self.bot.dynamic_lot_sizing, self.bot.risk_percent
                )
