...
def analyze_with_gemini_advanced(
    client: genai.Client, analysis: dict, df: pd.DataFrame, symbol: str
) -> dict:
    """
    ...
    price_change_5 = (
        ((df["close"].iloc[-1] / df["close"].iloc[-5]) - 1) * 100 if len(df) >= 5 else 0
    )
    price_change_20 = (
...
"""
    ...
    try:
        ...
        print(
            f"Raw response: {response.text[:200] if response.text else 'Empty response'}"
        )
        return {
            "recommendation": "WAIT",
            "confidence": 0,
...
def menu_1_analyze_now(config: dict, gemini_client: genai.Client = None) -> None:
    ...
    print(
        f"Mode: {config['current']['trade_mode']} | Threshold: {config['current'].get('min_signal_strength', 0.2):.0%}"
    )
    ...
    analyzer = MarketAnalyzer(
        news_api_key=env.get("news_api_key"), te_key=env.get("trading_economics_key")
    )
    ...
    print(
        f"Signal: {tech.get('signal')} | Bullish: {tech.get('bullish')} | Bearish: {tech.get('bearish')}"
    )
...
"""
            response = gemini_client.models.generate_content(
                model="gemini-2.5-flash", contents=quick_prompt
            )
            if response.text:
    ...
    print(
        f"Signal: {overall.get('signal')} | Strength: {overall.get('strength', 0):.1%}"
    )
...
def menu_7_launch_trainer(config: dict) -> None:
    ...
    try:
        ...
        fig.suptitle(
            f"Trading Trainer - {config['current']['symbol']} ({config['current']['timeframe']})"
        )

        def update_chart(frame):
            df = ambil_candle(
                config["current"]["symbol"],
                config["current"]["timeframe"],
                config["current"]["candles"],
            )
            if df.empty:
                return
            ...
            ax1.plot(
                df["time"], df["SMA_20"], label="SMA 20", color="orange", alpha=0.7
            )
            ...
            colors = [
                "g" if df["close"].iloc[i] > df["open"].iloc[i] else "r"
