        ...
        try:
            for trigger in price_triggers:
                ...
                if triggered:
                    ...
                    if trigger['sl']: request['sl'] = trigger['sl']
                    if trigger['tp']: request['tp'] = trigger['tp']
...
def menu_22_cancel_price_trigger() -> None:
    ...
    try:
        ...
        if trigger_id.lower() == 'all':
            cancelled = sum(1 for t in price_triggers if t['status'] == 'PENDING' and (t.update({'status': 'CANCELLED'}) or True))
