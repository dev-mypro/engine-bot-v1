...
def menu_2_change_symbol(config: dict) -> dict:
    ...
    if all_symbols:
        ...
        popular = [
            ...
            if any(pair in s.name for pair in common_symbols)
        ]
        if popular:
            print("\\nPopular symbols:")
            for sym in sorted(popular)[:5]:
                ...
                if sym_info:
                    print(
                        f"- {sym} (Bid: {sym_info.bid:.5f} | Ask: {sym_info.ask:.5f})"
                    )

    new_symbol = input(
        f"\\nMasukkan symbol baru (e.g., EURUSD, XAUUSD, XAUUSDm): "
    ).strip()
...
def menu_3_change_timeframe(config: dict) -> dict:
    new_tf = (
        input(
            f"\\nMasukkan timeframe baru (Opsi: {', '.join(config['options']['timeframes'])}): "
        )
        .strip()
        .upper()
    )
...
def menu_5_switch_account(config: dict) -> dict:
    new_acc = (
        input(
            f"\\nMasukkan tipe akun (Opsi: {', '.join(config['options']['accounts'])}): "
        )
        .strip()
        .upper()
    )
...
def menu_6_change_trade_mode(config: dict) -> dict:
    new_mode = (
        input(
            f"\\nMasukkan mode trading (Opsi: {', '.join(config['options']['trade_modes'])}): "
        )
        .strip()
        .upper()
    )
...
def menu_8_toggle_auto_trade(config: dict) -> dict:
    ...
    save_config(config)
    print(
        f"✅ Auto Trade diubah menjadi: {'ON' if config['current']['auto_trade'] else 'OFF'}"
    )
    return config
...
def menu_11_toggle_auto_close_profit(config: dict) -> dict:
    ...
    save_config(config)
    print(
        f"✅ Auto Close Profit diubah menjadi: {'ON' if config['current']['auto_close_profit'] else 'OFF'}"
    )
    return config
...
def menu_13_toggle_auto_analyze(config: dict) -> dict:
    ...
    save_config(config)
    print(
        f"✅ Auto Analyze diubah menjadi: {'ON' if config['current']['auto_analyze'] else 'OFF'}"
    )
    return config
...
def menu_18_toggle_stpp_trailing(config: dict) -> dict:
    ...
    save_config(config)
    print(
        f"✅ STPP Trailing diubah menjadi: {'ON' if config['current']['stpp_trailing'] else 'OFF'}"
    )
    return config
...
def menu_23_set_entry_decimals(config: dict) -> dict:
    try:
        ...
        save_config(config)
        print(
            f"✅ Entry Decimals diatur menjadi: {config['current']['entry_decimals']}"
        )
    except ValueError:
...
def menu_29_toggle_trade_always_on(config: dict) -> dict:
    config["current"]["trade_always_on"] = not config["current"].get(
        "trade_always_on", False
    )
    save_config(config)
    print(
        f"✅ Trade Always On diubah menjadi: {'ON' if config['current']['trade_always_on'] else 'OFF'}"
    )
    return config
...
def menu_30_change_mode_settings(config: dict) -> dict:
    ...
    print(
        "2) ⚡ AGGRESSIVE   - Fast signals (10% threshold, 2min check, 50 trades/day)"
    )
    ...
    if choice in presets:
        ...
        try:
            ...
            if (
                ...
                config["current"]["enable_scalping"] = (
                    input("Enable scalping signals? (y/n): ").lower() == "y"
                )
                config["current"]["enable_pattern_trading"] = (
                    input("Enable pattern trading? (y/n): ").lower() == "y"
                )
                config["current"]["enable_breakout_trading"] = (
                    input("Enable breakout trading? (y/n): ").lower() == "y"
                )
...
def menu_31_setup_multi_position(config: dict) -> dict:
    ...
    if choice in presets:
        ...
        try:
            config["current"]["max_positions_per_symbol"] = int(
                input("Max positions per symbol (1-20): ")
            )
            config["current"]["max_total_positions"] = int(
                input("Max total positions (1-100): ")
            )
            config["current"]["enable_multi_symbol"] = (
                input("Enable multi-symbol? (y/n): ").lower() == "y"
            )
            config["current"]["rapid_fire_mode"] = (
                input("Enable rapid fire mode? (y/n): ").lower() == "y"
            )
            save_config(config)
            print("\\n✅ Custom settings saved!")
        except:
